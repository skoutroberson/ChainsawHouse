...
void APortalRoom::TeleportPlayer(UPARAM(ref)AActor * TargetRoom, UPARAM(ref)AActor * Player)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (i < OverlappingGrabbables.Num())
		{
			...
			OverlappingGrabbables[i]->SetActorLocationAndRotation(GrabTargetLocation, GrabTeleportRotation, false, nullptr, ETeleportType::TeleportPhysics);
			//counter++;
		}
	}
	...
	VRChar->GetCharacterMovement()->Velocity.Set(NewVelocity.X, NewVelocity.Y, NewVelocity.Z);
	PlayerController->SetControlRotation(TeleportRotation);
	Player->SetActorLocationAndRotation(TargetLocation, TeleportRotation, false, nullptr, ETeleportType::TeleportPhysics);
