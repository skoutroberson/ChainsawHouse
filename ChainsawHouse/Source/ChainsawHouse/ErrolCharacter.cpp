...
void AErrolCharacter::BeginPlay()
{
	...
	//OpenDoorQueryParams.bTraceComplex = true;
...
void AErrolCharacter::EnterChaseState(float MaxSpeed, float ChaseDuration)
{
	...
	EPathFollowingRequestResult::Type MoveResult = ErrolController->MoveToActor(Player, 10.f, false, true, false, 0, false); //KillRadius);
	World->GetTimerManager().UnPauseTimer(OpenBlockingDoorTimer);

	UE_LOG(LogTemp, Warning, TEXT("MoveToPlayer: %d"), MoveResult);
...
void AErrolCharacter::TickChaseState(float DeltaTime)
{
	...
	{
		HaveWeBeenSeen();
		if (bSeenByPlayer) // && bChaseWhenSeen
		{
			// set speed to run
			if (bSprintAfterSeen)
			{
	...
	if (bKillImmediately)
	{
		if (State == ErrolState::STATE_CHASE && Dist < ChaseKillDistance)
		{
			
			// only kill player if the player is on the nav mesh (not in crawlspace or understairs tunnel)

			const UCapsuleComponent *PlayerCapsule = Cast<UCapsuleComponent>(Player->GetComponentByClass(UCapsuleComponent::StaticClass()));
			const float CapsuleHalfHeight = PlayerCapsule->GetScaledCapsuleHalfHeight();
			FVector PL = Player->GetActorLocation();
			PL.Z -= CapsuleHalfHeight;
			const FVector ProjNavPoint = NavigationSystem->ProjectPointToNavigation(World, PL);

			Dist = FVector::Dist(PL, ProjNavPoint);
...
void AErrolCharacter::OpenDoorBlockingPath()
{
	...
	//DrawDebugLine(World, AL, AL + GetActorForwardVector() * 200.f, FColor::White, false, World->DeltaTimeSeconds * 1.1f);

	//UE_LOG(LogTemp, Warning, TEXT("n : %d"), n);

	//NavigationSystem->FindPath

	// this stopped working all of the sudden after changing project collision complexity in .ini and i think it broke this?!
	//UPDATE: actually i changed the nav agent query in the project settings when i was working on another thing. I deleted it and now this works.
	...
	if (n >= 2)
	{
		...
		for (int i = n-1; i > n - 4; --i)
		{
			...
			//DrawDebugLine(World, P1, P1 + Dir * SegmentDistance, FColor::Magenta, false, 1.1f);
...
void AErrolCharacter::KillPlayer()
{
	...
	VRChar->LeftController->Release();
	VRChar->RightController->Release();

	VRChar->DisablePauseGame();
...
void AErrolCharacter::ExitPeekState()
{
	...
	bPeeking = false;
	bPeekOnScreen = false;
	LastPeekScareLevel = PeekScareLevel;
	PeekScareLevel = 0;
...
void AErrolCharacter::ShouldEndPeek(float DeltaTime)
{
	...
	if (bPeekOnScreen && !bPeekCurrentlyOnScreen && PeekScareLevel > 15.0f)
	{
	...
	// stop peek if the player moves too close to the peek
	if (DSize < PeekDistance * 0.66f)
	{
		bPeeking = false;
		EndPeek();
		return;
