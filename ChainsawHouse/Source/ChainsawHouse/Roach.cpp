...
void ARoach::Tick(float DeltaTime)
{
	...
	/**
		...
		I think this if statement will solve a bug that has been confusing me randomly for months
		I would notice that a roach would be bugged floating and it wouldn't make sense but now I see
		that in high performance situations, the roach would tick off screen at a lower frame rate.
		capping the framerate with this one line of beautiful code (looks better as if statement IMO)
		//DeltaTime = (DeltaTime > 0.0139) ? 0.0139 : DeltaTime;
	*/
...
void ARoach::TickMoveState(float DeltaTime)
{
	...
	{
		//UE_LOG(LogTemp, Warning, TEXT("Turning2"));
		Turn(DeltaTime);
		bTurn = false;
	}
	else if(CheckDown(DeltaTime))
	{
		...
		if (bClimbDown)
		{
			...
			//UE_LOG(LogTemp, Warning, TEXT("Turning3"));
	...
	{
		//UE_LOG(LogTemp, Warning, TEXT("Last Turn: %s"), *GetName());
		//bTurn = true;
		Turn(DeltaTime);
	}
...
*/
	...
	bool bDownTrace = World->LineTraceSingleByChannel(HitResult, TS, TE, ECollisionChannel::ECC_WorldDynamic, QueryParams);

	//DrawDebugLine(World, TS, TE, FColor::Red, false, DeltaTime * 1.1f);
	...
	else
	{
		...
		while (DownTraceIterations < MaxDownTraceIterations)
		{
			...
			bDownTrace = World->LineTraceSingleByChannel(HitResult, TS, TE, ECollisionChannel::ECC_WorldDynamic, QueryParams);
...
void ARoach::Swerve(float DeltaTime)
{
	...
	SetActorRotation(FMath::QInterpTo(ActorQuat, NewQuat, DeltaTime, SwerveSpeed *CopulateRotateSpeedModifier * (1.0f + (FleeSpeedModifier * 0.1f))));
}
...
void ARoach::ChangeSwerveSpeed()
{
	...
	if (bFlee)
	{
		SwerveSpeed += FleeSpeedModifier * 0.25f;
		//SwerveSpeed *= FleeSpeedModifier;
	}
...
void ARoach::WaitIfRolled()
{
	...
	if (bRoll)
	{
		...
		*/
		...
		World->GetTimerManager().SetTimer(WaitTimerHandle, this, &ARoach::WaitIfRolled, 0.01f, false, WaitTime);
	}
	else
	{
