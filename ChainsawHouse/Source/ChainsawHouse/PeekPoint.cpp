...
APeekPoint::APeekPoint()
{
	...
	LeftPeekSpawn = CreateDefaultSubobject<USceneComponent>(TEXT("LeftPeekSpawn"));
	RightPeekSpawn = CreateDefaultSubobject<USceneComponent>(TEXT("RightPeekSpawn"));
	LeftPeekSpawn->AttachTo(PeekRoot);
	RightPeekSpawn->AttachTo(PeekRoot);
}
...
bool APeekPoint::IsValid(const float Threshold)
{
	...
	if (DotLook > Threshold)
	{
		...
		{
			DrawDebugLine(World, HeadLocation, CL, FColor::Emerald, false, World->DeltaTimeSeconds * 1.1f);
			if (HitResult.GetActor() != nullptr)
			{
				UE_LOG(LogTemp, Warning, TEXT("%s Trace Hit: %s"), *GetName(), *HitResult.GetActor()->GetName());
			}
		}
	}
