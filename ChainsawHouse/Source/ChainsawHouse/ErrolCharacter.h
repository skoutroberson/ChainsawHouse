...
UCLASS()
class CHAINSAWHOUSE_API AErrolCharacter : public ACharacter
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	UCurveFloat *PeekDoorCloseCurve;
	...
	// for checking if the peek was seen on EndPeek
	UPROPERTY(VisibleAnywhere, BlueprintReadWrite)
	int LastSeenPeeks = -1;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	float PeekDistance = 0.f;

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	bool bSprintAfterSeen = true;
...
// misc
	...
	// set to equal PeekScareLevel in ExitPeekState()
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	float LastPeekScareLevel = 0.0f;
