...
void ALightManager::Flicker(float DeltaTime)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (CurrentFlickerTime > FlickerTime && (CurrentFlickerTime - FlickerTime) < 5.f)
		{
			...
			//UE_LOG(LogTemp, Warning, TEXT("Flicker: %s"), *FlickerLight->LightActor->GetName());
			break;
		}
	}
...
void ALightManager::AddFlickerLight(AActor *LightActor, float SpeedCoefficient, float LightIntensity, float EmissivePower, FVector LightColor)
{
	...
	{
		const int n = FlickerLights.Num();

		for (int i = 0; i < n; ++i)
		{
			if (LightActor == FlickerLights[i]->LightActor)
			{
				UE_LOG(LogTemp, Warning, TEXT("Light already flickering: %s"), *LightActor->GetName());
				return;
			}
		}
		...
		RandTime = FMath::Clamp(RandTime, 0.04f, 5.0f);
