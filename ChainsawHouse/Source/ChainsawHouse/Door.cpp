...
void ADoor::BeginPlay()
{
	...
	*/
	...
	SqueakRadius = FMath::RandRange(0.5f, 1.2f);
	SqueakMidPoint = FMath::RandRange(SqueakRadius + 0.15f, 1.57f - SqueakRadius - 0.15f);
}
// Called every frame
...
void ADoor::Swing(float DeltaTime)
{
	...
	if (MaxDistance > MaxAngleRadians)
	{
		...
		if (!bFullyClosed)
		{
			...
			CloseSoundDelay(SwingVelocity);

			//UGameplayStatics::PlaySoundAtLocation(GetWorld(), CloseSound, Doorknob->GetComponentLocation(), FMath::Clamp(SwingVelocity * CloseAudioMultiplier, 0.1f, 1.0f));
			/*
			if (!bPlayingCloseSound)
			{
				UGameplayStatics::PlaySoundAtLocation(GetWorld(), CloseSound, Doorknob->GetComponentLocation(), FMath::Clamp(SwingVelocity * CloseAudioMultiplier, 0.1f, 1.0f));
				CloseSoundDelay();
			}
...
void ADoor::UseDoor(float DeltaTime)
{
	...
	SlerpSize = (-Dot * HCDelta.Size() * (180.f / PI)) * 0.00025f;
...
{
	if (CurrentDoorAngle > SqueakMidPoint - SqueakRadius && CurrentDoorAngle < SqueakMidPoint + SqueakRadius)
	{
		const float AbsVel = fabsf(Velocity);
		const float SwingVolumeMultiplier = FMath::Clamp(AbsVel * 40.f, 0.2f, 1.0f);
		const float SwingPitchMultiplier = FMath::Clamp(AbsVel * 100.f, 0.85f, 1.15f);
...
void ADoor::PassController(AActor * HC)
{
	if (!bLocked)
	{
		if (HC != nullptr)
		{
			...
			if (bOpenDoorUsingCurve)
			{
				bOpenDoorUsingCurve = false;
				CurrentCurveTime = 0.0f;
				bFullyClosed = false;	// probably don't need this but I'm adding it to be safe.
			}
		}
...
void ADoor::OpenDoorUsingCurve(float DeltaTime)
{
	...
	//UE_LOG(LogTemp, Warning, TEXT("DoorAng: %f"), CurrentDoorAngle);
	...
	*/
	if (MinDistance > MaxAngleRadians)
	{
		...
		OpenHitAudio->Stop();
		OpenHitAudio->SetVolumeMultiplier(0.75f);
		OpenHitAudio->Play();
