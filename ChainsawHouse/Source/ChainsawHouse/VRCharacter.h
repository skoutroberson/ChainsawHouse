...
UCLASS()
class CHAINSAWHOUSE_API AVRCharacter : public ACharacter
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadWrite)
	float LastSafeCapsuleHeight = -1.0f;

	UFUNCTION(BlueprintImplementableEvent, BlueprintCallable)
	void UpdateSafeTransform();

	FTimerHandle UpdateSafeTransformHandle;

	void HeadCollisionSolver(float DeltaTime);

	TArray<AActor*> HeadColIgnoreActors;

	UWorld *World = nullptr;

	USphereComponent *CamColSphere = nullptr;
	float CamColRadius = -1.0f;
	float CapsuleRadius = -1.0f;
	float CapsuleHeight = -1.0f;

	void CapsuleHeightCheck();

	UFUNCTION(BlueprintImplementableEvent, BlueprintCallable)
	void ShowCrouchWidget();

	UFUNCTION(BlueprintImplementableEvent, BlueprintCallable)
	void HideCrouchWidget();

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	bool bGamePaused = false;

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	bool bPauseDisabled = false;

	void PressMenu();

	// called when getting killed and when the game first starts
	UFUNCTION(BlueprintImplementableEvent, BlueprintCallable)
	void DisablePauseGame();

	// called when respawned
	UFUNCTION(BlueprintImplementableEvent, BlueprintCallable)
	void EnablePauseGame();
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadWrite)
	bool bPressingBPaused = false;

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
	bool bSnapTurn = false;

	bool bTurnSnapped = false;

	UPROPERTY(VisibleAnywhere, BlueprintReadWrite)
	bool bStartingGame = true;

	void FixCapsulePenetration();

	void UpdateCapsuleHeightFix(float DeltaTime);

	void UpdateCapsuleHeightTest();

	float CapsuleGoalHalfHeight = 88.0f;
	FVector GoalLocation;

	FVector CameraStuckPosition = FVector::ZeroVector;
	FQuat CameraStuckQuat;
	FRotator StuckRotation = FRotator::ZeroRotator;

	// to change CorrectCameraOffset behavior the frame after being stuck or paused
	UPROPERTY(BlueprintReadWrite)
	bool bWasJustStuck = false;

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	bool bGettingKilled = false;
