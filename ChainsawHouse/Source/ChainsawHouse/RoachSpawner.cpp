...
void ARoachSpawner::SpawnRoach()
{
	...
	if (RoachCount >= TargetRoachCount || RoachCount >= MaxRoachCount)
	{
	...
	if (bFleeOnSpawn)
	{
		...
		NR->bHasFleed = true;
		//NR->WaitIfRolled(); // 12/31/2023 might cause errors. 1/15/2024 I THINK THIS IS CAUSING ERRORS
	...
	for (int i = 0; i < NumRoachesMovePerFrame; ++i)
	{
		if (RoachesMoved < RoachesToMove && MoveRoachIndex < Roaches.Num())
		{
			...
			if (RoachToMove->CurrentCopulateState != CopulateState::STATE_COPULATING)
			{
				...
				RoachToMove->bHasFleed = !FMath::RandRange(0, 2);
...
FTransform ARoachSpawner::GetRandomSpawnLocationAndRotation()
{
	...
	const FVector SBUV = SpawnBox->GetUpVector();
	...
	if (!bUseSpawnDirection)
	{
		const float RandomAngle = FMath::FRand() * TWOPI;
		RotQuat = FQuat(SBRV, HALF_PI);
		Rot = RotQuat * SBQ;
	...
	{
		//Rot = SpawnDirection.ToOrientationQuat();
		//const float RandomAngle = FMath::FRandRange(-0.5f, 0.5f);
		//RotQuat = FQuat(SBUV, RandomAngle);
		//Rot = RotQuat * Rot;
		const float RandomAngle = FMath::FRandRange(-0.5f, 0.5f);
		RotQuat = FQuat(SBRV, HALF_PI);
		Rot = RotQuat * SBQ;
